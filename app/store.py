...
def load_character(series_id: str, key: str) -> dict[str, Any] | None:
    """One character by its file-stem key (a slug, or 'narrator')."""
    return read_json(characters_dir(series_id) / f"{key}.json", None)


# --------------------------------------------------------------------------- #
# character portraits (Gemini-generated concept art, one PNG per character)
# --------------------------------------------------------------------------- #
def character_portraits_dir(series_id: str) -> Path:
    return characters_dir(series_id) / "portraits"


def character_portrait_path(series_id: str, key: str) -> Path:
    return character_portraits_dir(series_id) / f"{key}.png"


def mark_character_portrait_stale(series_id: str, key: str) -> None:
    character = load_character(series_id, key)
    if character and character.get("portrait_generated_at"):
        character["portrait_stale"] = True
        save_character(series_id, character)


def mark_all_character_portraits_stale(series_id: str) -> None:
    """Genre/tone/setting changed — every cast member's art direction is affected."""
    for character in load_characters(series_id):
        if character.get("portrait_generated_at"):
            character["portrait_stale"] = True
            save_character(series_id, character)
...
def _normalise_percentages(values: dict[str, int], keys: list[str]) -> dict[str, int]:
    clean = {key: max(0, int(values.get(key, 0) or 0)) for key in keys}
    total = sum(clean.values())
    if total <= 0:
        base, remainder = divmod(100, len(keys))
        return {key: base + (1 if i < remainder else 0) for i, key in enumerate(keys)}
    scaled = {key: int(clean[key] * 100 / total) for key in keys}
    remainder = 100 - sum(scaled.values())
    order = sorted(keys, key=lambda key: clean[key], reverse=True)
...
def save_story_analysis(series_id: str, analysis: dict[str, Any]) -> dict[str, Any]:
    """Persist validated SWOT and chart data with exact percentage totals."""
    revision = int(load_index(series_id).get("revision", 1) or 1)
    genre_keys = ["action", "drama", "comedy", "sci_fi", "horror", "thriller", "romance"]
    distribution = _normalise_percentages(analysis.get("genre_distribution", {}), genre_keys)
    raw_themes = list(analysis.get("themes", []))[:4]
    theme_weights = _normalise_percentages(
        {str(i): row.get("percentage", 0) for i, row in enumerate(raw_themes)},
        [str(i) for i in range(len(raw_themes))],
    ) if raw_themes else {}
    themes = [
        {"label": row.get("label", f"Theme {i + 1}"), "percentage": theme_weights[str(i)]}
        for i, row in enumerate(raw_themes)
    ]
    while len(themes) < 4:
        themes.append({"label": f"Theme {len(themes) + 1}", "percentage": 0})
    if sum(item["percentage"] for item in themes) != 100:
        fixed = _normalise_percentages(
            {str(i): row["percentage"] for i, row in enumerate(themes)},
            [str(i) for i in range(4)],
        )
        for i, row in enumerate(themes):
            row["percentage"] = fixed[str(i)]

    now = _now()
    swot = {
        key: list(analysis.get(key, []))[:4]
        for key in ("strengths", "weaknesses", "opportunities", "threats")
    }
    swot.update({"source_revision": revision, "generated_at": now, "stale": False})
    write_json(blueprint_dir(series_id) / "swot.json", swot)

    genre_path = blueprint_dir(series_id) / "genre.json"
    genre = read_json(genre_path, {}) or {}
    genre.update({
        "description": analysis.get("genre_description", ""),
        "tags": list(analysis.get("genre_tags", []))[:4],
        "distribution": distribution,
        "source_revision": revision,
    })
    write_json(genre_path, genre)

    theme_path = blueprint_dir(series_id) / "theme.json"
    theme = read_json(theme_path, {}) or {}
    theme.update({
    ...
    return {"swot": swot, "genre": genre, "theme": theme}
