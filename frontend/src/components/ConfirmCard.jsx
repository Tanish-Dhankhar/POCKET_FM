...
export default function ConfirmCard({ initial, onConfirm, onBack, busy }) {
  ...
  const updateTag = (key, index, value) => update(key, (form[key] || ['', '', '', '']).map((tag, i) => i === index ? value : tag))
  const validTags = ['genre_tags', 'theme_tags'].every((key) => {
    const tags = (form[key] || []).map((tag) => tag.trim()).filter(Boolean)
    return tags.length === 4 && new Set(tags.map((tag) => tag.toLowerCase())).size === 4
  })
  ...
  return <section className="question-card">
    ...
    <div className="confirm-grid">
      ...
      <div className="confirm-narrator"><div><strong>Narrator</strong><span>Add a separate guiding voice</span></div><button type="button" aria-label="Toggle narrator" className={`toggle ${form.include_narrator ? 'on' : ''}`} onClick={() => update('include_narrator', !form.include_narrator)} /></div>
      ...
      {['genre_tags', 'theme_tags'].map((key) => <div className="field full" key={key}><label>{key === 'genre_tags' ? 'Genre tags' : 'Theme tags'} · exactly four</label><div className="tag-editor">{(form[key] || ['', '', '', '']).slice(0,4).map((tag,index) => <input key={index} className="input" value={tag} onChange={(e) => updateTag(key,index,e.target.value)} />)}</div></div>)}
    </div>
    <div className="question-actions"><button className="button ghost" onClick={onBack} disabled={busy}>Back</button><button className="button primary" onClick={() => onConfirm(form)} disabled={busy || !form.title?.trim() || !validTags}>{busy ? 'Saving…' : 'Build my series'}</button></div>
  </section>
}
